...
func (h *colorHandler) ServeHTTP(writer http.ResponseWriter, request *http.Request) {
	log.Println("info: color requested, responding with", getColor())
	fmt.Fprint(writer, getColor())
}

type pingHandler struct{}
func (h *pingHandler) ServeHTTP(writer http.ResponseWriter, request *http.Request) {
	log.Println("ping: reponding with HTTP 200")
	writer.WriteHeader(http.StatusOK)
}

func main() {
	log.Printf("starting %s colorteller, listening on port %s", getColor(), getServerPort())
